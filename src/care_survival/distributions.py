...
def get_time_part(dgp):
    ...
    return (TC_low, TC_high, Lambda_inv)


def get_space_part(dgp):
    if dgp in [1, 3, 5]:
        d = 1
        def f_0_func(x):
    ...
    return (d, f_tilde_funcs, f_0_func)


def get_distribution(dgp):
    (TC_low, TC_high, Lambda_inv) = get_time_part(dgp)
    (d, f_tilde_funcs, f_0_func) = get_space_part(dgp)
